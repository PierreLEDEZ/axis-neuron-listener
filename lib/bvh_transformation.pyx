...
cpdef np.ndarray[UINT_t, ndim=3] bvh2GeometricFeatures(np.ndarray[FLOAT_t, ndim=2] frames, list joints, list ignored_joints, list ignored_joints_index):
    ...
    cdef np.ndarray[UINT_t, ndim=3] final_SPMF = cv2.resize(SPMF, (236,118), interpolation=cv2.INTER_NEAREST)

    final_SPMF = cv2.cvtColor(final_SPMF, cv2.COLOR_BGR2HSV)
    cdef np.ndarray H,S,V
    H,S,V = cv2.split(final_SPMF)
    ahe = cv2.createCLAHE(clipLimit=CLIPLIMIT, tileGridSize=(8,8))
    V = ahe.apply(V)
    final_SPMF = cv2.merge([H,S,V])
    final_SPMF = cv2.cvtColor(final_SPMF, cv2.COLOR_HSV2BGR)
...
cpdef np.ndarray[UINT_t, ndim=3] bvh2GeometricFeaturesV2(np.ndarray[FLOAT_t, ndim=2] frames, list joints, list ignored_joints, list ignored_joints_index):
    ...
    cdef np.ndarray[UINT_t, ndim=3] final_SPMF = cv2.resize(SPMF, (224,224), interpolation=cv2.INTER_NEAREST)
